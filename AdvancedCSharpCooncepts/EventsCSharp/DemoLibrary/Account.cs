...
namespace DemoLibrary
{
    public class Account
    {
        ...
        // IReadOnlyList<string> does not allow user to make changes to the list it just throws it to read it
        ...
        public bool MakePayment(string paymentName, decimal amount, Account backupAccount = null)
        {
            ...
            {
                // We don't have enough money so we check to see if we have a back up account
                if (backupAccount != null)
                {
                    // Checks to see if we have enough money in the backup account
                    if ((backupAccount.Balance + Balance) >= amount)
                    {
                        // We have enough backup funds to transfer the amount to this account
